...
// Odoo 16.0's view_widgets registry (used for standalone <widget name="..."/>
// tags) — like every other Owl registry in Odoo (fields, systray, views) —
// expects an object with a "component" key, e.g. { component: MyComponent },
// NOT the component class on its own. Registering the raw class directly
// makes the <widget> renderer's `t-component="widgetInfo.component"` resolve
// to undefined, which crashes with exactly the "OwlError ... Cannot read
// properties of undefined (reading 'name')" seen when opening this form.
registry.category("view_widgets").add("aioa_register_domain", { component: AioaRegisterDomain });
